...
def happinessData():
    joined_results = session.query(Happiness, Bands, Codes).join(Bands, Happiness.Country == Bands.Country).join(Codes, Happiness.Country == Codes.Country).all() #filter(Happiness.Country == Bands.Country).filter(Happiness.Country == Codes.Country).all()
